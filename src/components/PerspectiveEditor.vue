...
function resizeCanvas() {
  ...
  const containerHeight = 600 // 固定高度，也可以根据需要设置
  ...
  img.onload = () => {
    ...
    if (backgroundCanvas.value) {
      ...
      const scale = Math.min(
        canvasWidth / 3 / img.width,
        canvasHeight / 3 / img.height
      )
...
function applyPerspectiveTransform() {
  ...
  try {
    ...
    const perspective = new PerspectiveTransform(fgCtx.value, offscreenCanvas.value as unknown as HTMLImageElement)
